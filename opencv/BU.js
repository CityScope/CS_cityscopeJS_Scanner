...
function canny(src) {
  ...
  cv.Canny(
    dstC1,
    dstC1,
    controls.cannyThreshold1,
    controls.cannyThreshold2,
    controls.cannyApertureSize,
    controls.cannyL2Gradient
  );
  return dstC1;
}
...
function adaptiveThreshold(src) {
  ...
  cv.adaptiveThreshold(
    ...
    cv.THRESH_BINARY,
    Number(controls.adaptiveBlockSize),
    2
  );
  mat.delete();
  return dstC1;
...
function bilateralFilter(src) {
  ...
  cv.bilateralFilter(
    mat,
    dstC3,
    controls.bilateralFilterDiameter,
    controls.bilateralFilterSigma,
    controls.bilateralFilterSigma,
    cv.BORDER_DEFAULT
  );
  mat.delete();
...
function contours(src) {
  ...
  cv.findContours(
    ...
    Number(controls.contoursMethod),
    { x: 0, y: 0 }
  );
  dstC3.delete();
...
function backprojection(src) {
  ...
  if (controls.backprojectionRangeLow < controls.backprojectionRangeHigh) {
    ranges = [
      controls.backprojectionRangeLow,
      controls.backprojectionRangeHigh
    ];
  } else {
    return src;
  }
...
function erosion(src) {
  ...
  cv.erode(
    ...
    { x: -1, y: -1 },
    1,
    Number(controls.erosionBorderType),
    color
  );
  kernel.delete();
  return dstC4;
...
function dilation(src) {
  ...
  cv.dilate(
    ...
    { x: -1, y: -1 },
    1,
    Number(controls.dilationBorderType),
    color
  );
  kernel.delete();
  return dstC4;
...
function morphology(src) {
  ...
  cv.morphologyEx(
    ...
    { x: -1, y: -1 },
    1,
    Number(controls.morphologyBorderType),
    color
  );
  kernel.delete();
  return dstC4;
...
function initUI() {
  ...
  adaptiveThreshold
    .add(controls, "adaptiveBlockSize", 3, 99, 1)
    .name("block size")
  ...
  gaussianBlur
    .add(controls, "gaussianBlurSize", 7, 99, 1)
    .name("kernel size")
    .onChange(function(value) {
  ...
  medianBlur
    .add(controls, "medianBlurSize", 3, 99, 1)
    .name("kernel size")
    .onChange(function(value) {
  ...
  bilateralFilter
    .add(controls, "bilateralFilterDiameter", 1, 15, 1)
    .name("diameter");
  bilateralFilter
    .add(controls, "bilateralFilterSigma", 1, 255, 1)
    .name("sigma");
  ...
  morphology
    ...
    .name("shape");
  morphology
    .add(controls, "morphologySize", 1, 15, 1)
    .name("kernel size")
    .onChange(function(value) {
      if (value % 2 === 0) controls.morphologySize = value + 1;
    });
  morphology
    .add(controls, "morphologyBorderType", {
  ...
  laplacian
    .add(controls, "laplacianSize", 1, 19, 1)
    .name("kernel size")
    .onChange(function(value) {
  ...
  canny
    .add(controls, "cannyApertureSize", 3, 7, 1)
    .name("aperture size")
    .onChange(function(value) {
  ...
  histograms
    .add(controls, "calcHist")
    .name(filters["calcHist"])
    .onChange(function() {
      closeLastFolder(null);
    });
  histograms
    .add(controls, "equalizeHist")
    .name(filters["equalizeHist"])
    .onChange(function() {
      closeLastFolder(null);
    });
  ...
  backprojection
    .add(controls, "backprojectionRangeLow", 0, 255, 1)
    .name("range low");
  backprojection
    .add(controls, "backprojectionRangeHigh", 0, 255, 1)
    .name("range high");
}
